	...
	public class OrderPaidMessageReceiver : ISubscriptionMessageReceiver<OrderPaidMessageFilter>
	{
		...
		private async Task HandleMessage(Message message, CancellationToken cancellationToken)
		{
			var orderPaidNotification =
				JsonConvert.DeserializeObject<OrderPaidNotification>(Encoding.UTF8.GetString(message.Body));

			await _mediator.Publish(orderPaidNotification, cancellationToken);
