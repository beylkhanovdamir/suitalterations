...
RUN dotnet build "SuitAlterations.csproj" -c Release -o /app/build

FROM build AS publish
RUN dotnet publish "SuitAlterations.csproj" -c Release -o /app/publish

FROM base AS final
WORKDIR /app
COPY --from=publish /app/publish .
ENTRYPOINT ["dotnet", "SuitAlterations.dll"]